...
export default function Sidebar({ onNavigate, currentPage, onSecretClick, user, isCollapsed, toggleCollapse }) {
  ...
  const navItems = [
    ...
    { id: 'contact', label: 'Contact', icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 00-2-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /> }
  ];
  ...
  return (
    <>
      ...
      {/* SIDEBAR */}
      <aside 
        className={`fixed inset-y-0 left-0 z-40 bg-white/95 dark:bg-[#0c0a09]/95 backdrop-blur-xl border-r border-stone-200 dark:border-stone-800 transform transition-all duration-300 ease-in-out flex flex-col
      ...
      >
        ...
        <button 
            ...
            className="hidden md:flex absolute -right-3 top-24 bg-white dark:bg-stone-800 border border-stone-200 dark:border-stone-700 rounded-full p-1.5 text-stone-500 hover:text-emerald-500 hover:border-emerald-500 transition-all shadow-md z-50"
        >
            {isCollapsed ? (
        ...
        {/* LOGO AREA */}
        <div className={`h-20 flex items-center border-b border-stone-100 dark:border-stone-800 transition-all duration-300 ${isCollapsed ? 'justify-center px-0' : 'px-6 gap-3'}`}>
            <div className="flex items-center gap-3 cursor-pointer" onClick={() => onNavigate('profile')}>
                ...
                {!isCollapsed && (
                    <div className="transition-opacity duration-300 overflow-hidden whitespace-nowrap">
                        <h1 className="font-bold text-stone-900 dark:text-white leading-tight text-lg">Pethum</h1>
        ...
        <nav className="flex-1 overflow-y-auto overflow-x-hidden py-6 px-3 space-y-2 custom-scrollbar">
            {navItems.map((item) => (
                ...
                >
                    <svg className={`w-5 h-5 shrink-0 transition-colors ${currentPage === item.id ? 'text-emerald-500' : 'text-stone-400 group-hover:text-stone-600 dark:group-hover:text-stone-300'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        {item.icon}
                    </svg>
        ...
        <div className="p-4 border-t border-stone-100 dark:border-stone-800 space-y-3">
            ...
            <button 
                ...
                className={`w-full flex items-center rounded-lg bg-stone-50 dark:bg-stone-800 text-stone-600 dark:text-stone-300 text-xs font-medium hover:bg-stone-100 dark:hover:bg-stone-700 transition-colors ${isCollapsed ? 'justify-center p-2.5' : 'justify-between px-3 py-2.5'}`}
            ...
            <button 
                ...
                className={`w-full flex items-center gap-3 rounded-lg text-xs font-bold transition-all shadow-sm
                ${isCollapsed ? 'justify-center p-2.5' : 'px-3 py-3'}
                ${user ? 'bg-emerald-600 text-white hover:bg-emerald-700' : 'bg-stone-900 dark:bg-white text-white dark:text-black hover:opacity-90'}
                `}
                title={!isCollapsed ? '' : (user ? 'Signed In' : 'Login')}
            >
                ...
                {!isCollapsed && (
                    <div className="flex flex-col items-start leading-none overflow-hidden">
                        <span className="opacity-70 font-normal text-[10px] whitespace-nowrap">{user ? 'Signed in as' : 'Admin Access'}</span>
                        <span className="truncate w-32 text-left">{getSafeName()}</span>
                    </div>
                )}
            </button>
        </div>
